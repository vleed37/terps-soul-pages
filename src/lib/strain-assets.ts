...
/** Shop-card photography used when a dedicated product image is unavailable. */
export const STRAIN_SHOP_IMAGE: Record<string, string> = {
...
export function getStrainProductImage(slug: string): string | undefined {
  return STRAIN_PRODUCT_IMAGE[slug] ?? STRAIN_SHOP_IMAGE[slug];
}
